"""
Python port of testing.marketing.integrity.HomepageLinksTest (simplified).

Java delegates to UrlValidationRunner; Python uses a sampled HEAD-check
scan instead. Same tolerance threshold.
"""
...
class TestHomepageLinks:
    ...
    def test_homepage_internal_links_reachable(self) -> None:
        ...
        for url in sample:
            try:
                ...
                pass
        logger.info("Homepage link scan: %d total, %d internal sampled, %d broken",
                    len(hrefs), len(sample), len(broken))
        assert len(broken) <= MAX_HIGH_SEVERITY_FINDINGS, (
            f"Homepage produced {len(broken)} broken-link findings "
